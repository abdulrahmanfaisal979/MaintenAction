...
# End-of-run "likely but not confirmed" evidence.
# These do NOT create confirmed action events.
FAST_INSERT_PARTIAL_WINDOWS = 3
FAST_HAND_PARTIAL_WINDOWS = 2
FAST_ACTION_HINT_MIN_CONFIDENCE = 0.85
...
.ma-sensor-ribbon {
    ...
    display:grid;grid-template-columns:1.35fr .9fr .9fr .85fr;gap:0;
...
.ma-likely-hint {
    ...
    padding:10px 12px;
    color:#ffd783;
    background:linear-gradient(180deg,rgba(82,56,11,.78),rgba(37,25,6,.90));
    border:1px solid rgba(255,175,22,.72);
    box-shadow:inset 0 0 0 2px #020c14,0 0 12px rgba(255,175,22,.14);
    font-family:"Barlow Condensed",sans-serif;
    font-size:.96rem;
    font-weight:700;
    line-height:1.2;
}
.ma-likely-hint b {color:#fff0c5;font-weight:800;}
...
@media(max-width:1180px) {
    ...
    .ma-live-title{font-size:2rem}.ma-sensor-ribbon{grid-template-columns:1fr 1fr 1fr}.ma-ribbon-cell:nth-child(4){display:none;}
}
...
def finish_fast_action_streak():
    """Save strong partial evidence without confirming an action."""
    action = st.session_state.fast_action_streak_action
    confidences = list(st.session_state.fast_action_streak_confidences)

    if action not in {"insert_bolt_nut", "hand_tighten"} or not confidences:
        st.session_state.fast_action_streak_action = None
        st.session_state.fast_action_streak_confidences = []
        return

    votes = len(confidences)
    required = (
        INSERT_BOLT_CONFIRMATION_WINDOWS
        if action == "insert_bolt_nut"
        else CONFIRMATION_REQUIRED["hand_tighten"]
    )
    partial_votes = (
        FAST_INSERT_PARTIAL_WINDOWS
        if action == "insert_bolt_nut"
        else FAST_HAND_PARTIAL_WINDOWS
    )
    ...
    is_strong_likely = (
        votes == partial_votes
        and votes < required
        and avg_confidence > FAST_ACTION_HINT_MIN_CONFIDENCE
    )

    if is_strong_likely:
        # If hand-tighten is strongly likely, that is enough evidence that
        # the previous insert action ended. It releases a repeated insert,
        # but does NOT record hand-tighten as a confirmed event.
        if (
            action == "hand_tighten"
            and st.session_state.last_counted_action == "insert_bolt_nut"
        ):
            st.session_state.released_since_last_event = True

        # The yellow end warning is only relevant to a missing #4.
        # Therefore save evidence only when #1-#3 were already confirmed.
        if int(st.session_state.action_counts.get(action, 0)) == 3:
            st.session_state.likely_missing_fourth[action] = {
                "votes": int(votes),
...
def reset_monitoring_state():
    ...
    st.session_state.fast_action_streak_action = None
    st.session_state.fast_action_streak_confidences = []
    st.session_state.likely_missing_fourth = {}
...
def render_procedure_compliance(show_details=False, show_compact=True):
    ...
    if not st.session_state.running:
        ...
        for action in ("insert_bolt_nut", "hand_tighten"):
            row = next(
                (r for r in result["completion_rows"] if r["Action"] == ACTION_DISPLAY[action]),
                None,
            )
            evidence = st.session_state.likely_missing_fourth.get(action)

            if (
                row is not None
                and int(row["Required"]) == 4
                and int(row["Detected"]) == 3
                and evidence is not None
            ):
                warning_parts.append(
                    f'{html.escape(ACTION_DISPLAY[action])} #4 was '
                    f'<b>LIKELY BUT NOT CONFIRMED</b> '
                    f'({int(evidence["votes"])}/{int(evidence["required"])} votes, '
                    f'{float(evidence["confidence"]):.0%} avg confidence)'
                )

        if warning_parts:
            likely_warning_html = (
                '<div class="ma-likely-hint">'
                '<b>LIKELY MISSED ACTION</b><br>'
                + '<br>'.join(warning_parts)
                + '</div>'
            )
...
init_state("fast_action_streak_action", None)
init_state("fast_action_streak_confidences", [])
init_state("likely_missing_fourth", {})
...
with control_2:
    ...
    if st.button("■  STOP MONITORING", use_container_width=True,):
        ...
        # Preserve a final strong partial insert/hand attempt for the
        # end-of-run likely warning before monitoring is frozen.
        finish_fast_action_streak()
...
def live_monitor():
    ...
    if should_predict:
        ...
        if prediction_allowed_to_vote:
            # Track only a separate short streak for the requested likely fallback.
            # This does not alter normal confirmation voting.
            if prediction in {"insert_bolt_nut", "hand_tighten"}:
                if st.session_state.fast_action_streak_action == prediction:
                    st.session_state.fast_action_streak_confidences.append(float(confidence))
                else:
                    finish_fast_action_streak()
                    st.session_state.fast_action_streak_action = prediction
                    st.session_state.fast_action_streak_confidences = [float(confidence)]
            else:
                finish_fast_action_streak()
            ...
            if candidate == "idle":
                ...
                if (combined_confidence >= MIN_COMBINED_CONFIDENCE):
                    ...
                    if events_after > events_before:
                        ...
                        if candidate in {"insert_bolt_nut", "hand_tighten"}:
                            st.session_state.fast_action_streak_action = None
                            st.session_state.fast_action_streak_confidences = []
